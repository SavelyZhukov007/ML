...
def load_real_estate_data(num_samples=1000):
    ...
    X_real = np.column_stack(
        (
            np.random.uniform(30, 250, num_samples),    
            np.random.randint(1, 6, num_samples),       
            np.random.randint(1, 25, num_samples),      
            np.random.uniform(0.5, 40, num_samples),    
            np.random.uniform(1990, 2024, num_samples), 
            np.random.randint(0, 2, num_samples),       
        )
    )
...
show_spinner("Генерируем Датасет")
np.random.seed(42)
num_samples = 100000
...
print(f"Всего {num_samples} записей")
X_real, y_real = load_real_estate_data(num_samples=10000)
show_spinner("Строим модель")
model = Sequential(
    [
...
show_spinner("Создание графиков", 2)
fig = plt.figure(figsize=(20, 12))
fig.suptitle(
    "Анализ точности модели: Синтетические vs Реальные данные",
    fontsize=16,
    fontweight="bold",
)
...
ax2.plot(
    [y_synthetic_sample.min() / 1e6, y_synthetic_sample.max() / 1e6],
    [y_synthetic_sample.min() / 1e6, y_synthetic_sample.max() / 1e6],
    "r--",
    lw=2,
)
...
ax3.hist(
    [errors_real, errors_synthetic],
    bins=50,
    label=["Реальные", "Синтетические"],
    alpha=0.6,
)
...
ax4.scatter(
    X_real[:, 0], y_pred_real / 1e6, alpha=0.3, s=10, label="Предсказания", color="red"
)
ax4.set_xlabel("Площадь (м^2)")
ax4.set_ylabel("Цена в млн руб")
ax4.set_title("Площадь на Цену")
ax4.legend()
...
ax6.scatter(
    X_real[:, 2], y_pred_real / 1e6, alpha=0.3, s=10, color="red", label="Предсказания"
)
ax6.set_xlabel("Этаж")
...
ax7.scatter(
    X_real[:, 3], y_pred_real / 1e6, alpha=0.3, s=10, color="red", label="Предсказания"
)
...
ax8.scatter(
    X_real[:, 4], y_pred_real / 1e6, alpha=0.3, s=10, color="red", label="Предсказания"
)
